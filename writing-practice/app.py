...
class GermanLearningApp:
    def __init__(self):
        logger.debug("Initializing German Learning App...")
    ...
    def generate_sentence(self, word: dict) -> str:
        ...
        prompt = f"""Generate a simple German sentence using the word '{german}'.
        The grammar should be scoped to German A1 certification grammar.
        You can use the following vocabulary to construct a simple sentence:
        - simple objects eg. book, car, steak, sushi
        - simple verbs, to drink, to eat, to meet
        - simple times eg. tomorrow, today, yesterday
        
        Please provide the response in this format:
        German: [sentence in German]
        English: [English translation]
        """
        
        logger.debug(f"Generating sentence for word: {german}")
    ...
    def grade_submission(self, image) -> Dict:
        ...
        return {
            "transcription": "Ich werde heute Steak essen.",
            "translation": "I will eat steak today",
            "grade": "S",
    ...
    def render_setup_state(self):
        ...
        st.title("German Writing Practice")
        ...
        if generate_button:
            ...
            logger.debug(f"Selected word: {word.get('english')} - {word.get('german')}")
    ...
    def render_practice_state(self):
        ...
        st.title("Practice German")
        st.write(f"English Sentence: {st.session_state.current_sentence}")
        
        uploaded_file = st.file_uploader("Upload your written German", type=['png', 'jpg', 'jpeg'])
