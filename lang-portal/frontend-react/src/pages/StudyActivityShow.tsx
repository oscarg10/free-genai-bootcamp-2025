...
export default function StudyActivityShow() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null)
      try {
        const response = await fetch(`http://localhost:5100/api/study-activities/${id}`)
        if (!response.ok) {
        ...
        const sessionsResponse = await fetch(
          `http://localhost:5100/api/study-activities/${id}/sessions?page=${currentPage}&per_page=${ITEMS_PER_PAGE}`
        )
